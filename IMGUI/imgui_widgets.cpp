...
#include <string>
#include <map>
#include "../MentalityUI/custom_widgets_ios.h"
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	if (axis == ImGuiAxis_X)
	 ...
	 window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, c::anim::active, c::anim::active, c::dark_color, c::dark_color, 30.f);
...
} // namespace ImStb (temporarily close for includes)
#include <map>
#include <string>
namespace ImStb {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? label_size.x : 0.0f), size_arg.y);

	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, size_arg.y));
	...
	const bool hovered = ItemHoverable(frame_bb, id, (ImGuiItemFlags)0);
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::anim::active : hovered ? c::text::label::hovered : c::text::label::default_color, g.IO.DeltaTime * 6.f);

	if (!is_multiline)
	{
		GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, c::anim::default_color, c::elements::rounding);
		//GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, c::stroke_color, c::elements::rounding);
	}

	const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + w, frame_bb.Min.y + inner_size.y);
	ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2(frame_bb.Min.x + (42 - CalcTextSize(hint).y) / 2, frame_bb.GetCenter().y - CalcTextSize(label).y / 2);
	...
	if (render_cursor || render_selection)
	{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					if (rect.Overlaps(clip_rect))
						window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32((ImVec4)c::anim::active, 0.2f), c::elements::rounding);
				}
		...
		{
				ImU32 col = is_displaying_hint ? c::label::default_color: c::label::active;
													  window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(it_anim->second.text), buf_display, buf_display_end, 0.0f, !is_multiline ? &clip_rect : NULL);
		}
		...
		if (render_cursor)
		{
			...
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
				draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + it_anim->second.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + it_anim->second.slow, cursor_screen_rect.GetBL().y), c::label::active);
	...
	else
	{
		...
		if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
		{
			ImU32 col = is_displaying_hint ? c::label::active : c::label::active;
			draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32(it_anim->second.text), buf_display, buf_display_end, 0.0f, !is_multiline ? &clip_rect : NULL);
		}
	}
	...
	it_anim->second.label = ImLerp(it_anim->second.label, CalcTextSize(buf_display).x < 1.f ? c::text::label::default_color: utils::GetColorWithAlpha(c::text::label::default_color, 0.f), g.IO.DeltaTime * 6.f);

																													 GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x + 14.f, frame_bb.GetCenter().y - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.label), label);
